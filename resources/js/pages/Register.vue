            ...
            <ValidationObserver v-slot="{ handleSubmit }">
                <form @submit.prevent="handleSubmit(register)" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <transition name="validateError">
                        <div v-if="registerErrorMessages" class="mb-6">
                            <div v-if="registerErrorMessages.name">
                                <p v-for="msg in registerErrorMessages.name" :key="msg" class="text-red-500 text-xs italic">
                                    {{msg}}</p>
                            </div>
                            <div v-if="registerErrorMessages.email">
                                <p v-for="msg in registerErrorMessages.email" :key="msg" class="text-red-500 text-xs italic">
                                    {{msg}}</p>
                            </div>

                            <div v-if="registerErrorMessages.password">
                                <p v-for="msg in registerErrorMessages.password" :key="msg" class="text-red-500 text-xs italic">
                                    {{msg}}</p>
                            </div>
                        </div>
                    </transition>
                    ...
                    <div class="mb-6">
                        ...
                        <ValidationProvider name="password_confirm" rules="required" v-slot="{errors}">
                            <input
