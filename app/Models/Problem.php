...
use App\Traits\RelatedToFilePath;

class Problem extends Model
{
    use RelatedToFilePath;
    ...
    {
        return $this->belongsTo('App\Models\ExerciseBook');
    }
    ...
    public function newRegister($request, $exercise_book)
    {
        ...
        if ($problem_img && !$answer_img) {
            ...
            $this->saveFile($problem_img, $problem_img_path);
            $this->fill([
        ...
        if ($answer_img && !$problem_img) {
            ...
            $this->saveFile($answer_img, $answer_img_path);
            $this->fill([
                'content' => $request->problem,
                'answer' => $request->answer,
                'url' => $request->url,
    ...
    public function problemUpdate($problem, $request, $exercise_book)
    {
        ...
        if ($request->problem_image !== $problem->problem_img && $request->problem_image !== 'null') {
            ...
            $this->saveFile($request->problem_image, $problem_img_path);
        ...
        if ($request->answer_image !== $problem->answer_img && $request->answer_image !== 'null') {
            ...
            $this->saveFile($request->answer_image, $answer_img_path);
    ...
    public function addImageUrl($problem)
    {
        ...
        if ($problem_img && !$answer_img) {
            $problem['problem_img_url'] = $this->urlFetch($problem_img);
            return $problem;
        }
        if ($answer_img && !$problem_img) {
            $problem['answer_img_url'] = $this->urlFetch($answer_img);
            return $problem;
        }
        if ($answer_img && $problem_img) {
            $problem['problem_img_url'] = $this->urlFetch($problem_img);
